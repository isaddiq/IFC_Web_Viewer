...
function __flattenIfcObject(value, prefix = '', rows = [], seen = new WeakSet(), depth = 0) {
  if (value == null || depth > 4) return rows;
  ...
  if (Array.isArray(value)) {
    ...
    value.forEach((item, index) => {
      ...
      __flattenIfcObject(item, nextPrefix, rows, seen, depth + 1);
    });
    return rows;
  }
  ...
  for (const [key, nextValue] of Object.entries(value)) {
    if (ignoreKeys.has(key) || typeof nextValue === 'function' || nextValue == null) {
      continue;
    }
    const nextPrefix = prefix ? `${prefix}.${key}` : key;
    if (__isInlineIfcValue(nextValue)) {
      const text = __stringifyIfcValue(nextValue, depth + 1);
      if (text) rows.push({ name: nextPrefix, value: text, unit: '' });
    } else {
      __flattenIfcObject(nextValue, nextPrefix, rows, seen, depth + 1);
    }
  }
...
function __renderRowsSection(mount, title, rows) {
  const visibleRows = __dedupeRows(rows).filter(
    (row) => row && row.name && (row.value || row.unit),
  );
  ...
  if (!mount.children.length) {
    ...
    async function click_act(e, currentDownMove) {
      ...
      if (intersects) {
        ...
        renderOtherAndDimensions(prpsets);
